...
const SharePopup: React.FC<SharePopupProps> = ({ storyLink, onClose }) => {
    const [isPublic, setIsPublic] = useState(false);
    const [publicSwitcherOn, setPublicSwitcherOn] = useState(true);
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-lg relative">
                ...
                <div className="my-4">
                    <label className="flex items-center">
                        <Switch
                            ...
                            id="material-switch"
                        />
                        <p className="text-center ml-2">Сделать историю доступной всем</p>
                    </label>
                    {!publicSwitcherOn && (
                        <p className="absolute text-red-500 text-center mt-2 test-sm">Другие не смогут получить доступ к этой истории</p>
                    )}
                </div>
                <div className="my-10 pt-10 flex items-center">
                    <input
                        type="text"
                        value={storyLink}
                        readOnly
                ...
                {showCopiedMessage && (
                    <div className="absolute bottom-14 left-0 right-0 text-green-500 text-center mb-4">
                ...
                <div className="flex justify-between mt-4">
                    <button
                        onClick={onClose}
                        className="bg-gray-300 text-black w-full py-2 px-4 rounded"
                    >
                        Закрыть
                    </button>
                </div>
