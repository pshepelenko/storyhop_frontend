...
export function practiceCopy(language: UiLanguage) {
  if (language === 'russian') {
    return {
      ...
      skipFailed: 'Не удалось сохранить пропуск. Проверьте интернет и попробуйте ещё раз.',
      ...
      writingRewardCap: (count: number) => `До +${count} кристаллов`,
      writingIntroWordCount: (count: number) => `${count} слов`,
      writingIntroListenStep: 'Слушай слово и читай простое объяснение',
      writingIntroWriteStep: 'Пиши ответ и получай кристаллы',
      writingRevealAnswer: 'Показать ответ - 0 кристаллов',
      writingSkipNote: 'Можно пропустить и продолжить историю',
      writingProgress: (current: number, total: number) => `${current} из ${total} слов`,
      playWord: 'Прослушать слово',
      goHome: 'На главную',
      crystalCount: (count: number) => `${count} кристалл${count === 1 ? '' : count > 1 && count < 5 ? 'а' : 'ов'}`,
      phrasesWaiting: (count: number, maxReward: number) => `Фраз для практики: ${count}, до +${maxReward} кристаллов`,
      wordsWaiting: (count: number, maxReward: number) => `Слов для практики: ${count}, до +${maxReward} кристаллов`,
  ...
  return {
    ...
    skipFailed: 'Could not save the skip. Check your connection and try again.',
    listen: 'Listen',
    listening: 'Listening...',
    ...
    writingRewardCap: (count: number) => `Up to +${count} crystals`,
    writingIntroWordCount: (count: number) => `${count} words`,
    writingIntroListenStep: 'Listen to the word and read a simple explanation',
    writingIntroWriteStep: 'Write the answer and earn crystals',
    writingRevealAnswer: 'Show answer - 0 crystals',
    writingSkipNote: 'You can skip this and continue the story',
    writingProgress: (current: number, total: number) => `${current} of ${total} words`,
    playWord: 'Play word',
    goHome: 'Go home',
    crystalCount: (count: number) => `${count} ${count === 1 ? 'crystal' : 'crystals'}`,
    phrasesWaiting: (count: number, maxReward: number) => `${count} phrases waiting, up to +${maxReward} crystals`,
    wordsWaiting: (count: number, maxReward: number) => `${count} words, up to +${maxReward} crystals`,
